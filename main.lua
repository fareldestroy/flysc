...
FlyTab:Paragraph({ Title = "Fly Speed", Desc = "> 0  |  Max 300  |  Mobile support aktif" })
...
FlyTab:Paragraph({ Title = "Keybind", Desc = "Tekan F untuk toggle fly di PC.\nMobile pakai toggle UI." })
...
PlayerTab:Toggle({
    ...
    Callback = function(v)
        ...
        if godEnabled then
            ...
            -- Keep health maxed via connection
            godConnection = hum:GetPropertyChangedSignal("Health"):Connect(function()
                if godEnabled then
                    hum.Health = math.huge
                end
            end)
            WindUI:Notify({ Title = "God Mode ON", Content = "Karakter tidak bisa mati", Icon = "shield" })
        else
            if godConnection then godConnection:Disconnect(); godConnection = nil end
            hum.MaxHealth = 100
            hum.Health = 100
            WindUI:Notify({ Title = "God Mode OFF", Content = "HP kembali normal", Icon = "shield-off" })
        end
    end
})

-- Reset god mode saat respawn
